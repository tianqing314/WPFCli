...
{
    /// <summary>
    /// 注册设备层：共享标准盒/PLC（单例）、蓝牙扫描、环境温度、被检驱动注册表、设备提供者工厂。
    ...
    public static IServiceCollection AddPcbaDevices(this IServiceCollection services, HardwareOptions? hw = null)
    {
        ...
        // PLC：全局共享 -> 单例。真机走国锐 Modbus 驱动，否则仿真（与标准盒同一 useReal 开关）
        if (useReal)
        {
            services.AddSingleton<IPlcController, Plc.GuoRuiPlc>();
        }
        else
        {
            services.AddSingleton<IPlcController, Plc.SimulatedPlc>();
        }
