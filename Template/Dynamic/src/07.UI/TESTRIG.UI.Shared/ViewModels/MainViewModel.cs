...
public partial class MainViewModel : ObservableObject
{
    ...
    private async Task AutoUpdateLoopAsync()
    {
        ...
        while (true)
        {
            if (await _update.CheckAsync() && _update.HasUpdate)
            {
                ...
                // 全自动上下料是无人值守场景：弹窗会挡住产线画面且没人来点，只亮红点，
                // 等操作员停机回到工位自然会看到。手动检查更新不受此限制。
                if (!AnyBoardAutoMode)
                {
    ...
    /// <summary>
    /// 是否有板卡处于全自动（接入 PLC 上下料）模式——无人值守，升级提示不得弹窗。
    /// </summary>
    public bool AnyBoardAutoMode => _openBoards.Values.Any(b => b.AutoMode);
