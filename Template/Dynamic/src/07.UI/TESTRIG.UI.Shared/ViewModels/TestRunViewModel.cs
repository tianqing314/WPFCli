...
public partial class TestRunViewModel : ObservableObject
{
    ...
    /// <summary>
    /// 构造运行页：按号位构建进度视图、初始化连接/工位状态，并订阅执行器的进度/消息/采样事件。
    /// </summary>
    ...
    public TestRunViewModel(JigManifest manifest, TestRunner runner, ITestResultStore store,
        ConnectionManager conn, IConnectionConfigStore connStore, AutomationOrchestrator orch, IPlcController plc,
        ...
        HardwareOptions? hwOptions = null)
    {
        ...
        _scanner = scanner;
        _session = session;
        _autoMode = !_session.IsTestAccount;
        _dispatcher = Application.Current.Dispatcher;
        _useReal = hwOptions?.UseReal ?? false;
        ...
        PlcConnected = conn.IsPlcConnected;
        CurrentStation = plc.CurrentStation;
    ...
    /// <summary>
    /// 全自动运行状态变化：同步给板卡专属扩展（运行中禁改）。
    /// </summary>
    /// <param name="value">是否正在全自动运行。</param>
    ...
    /// <summary>
    /// 全自动开关：开 = 全自动测试（接入 PLC 自动上下料）；关 = 仅手动测试。默认打开。
    /// </summary>
    ...
    /// </summary>
    [ObservableProperty]
    [NotifyPropertyChangedFor(nameof(CurrentStationText))]
    [NotifyPropertyChangedFor(nameof(StationABg))]
    [NotifyPropertyChangedFor(nameof(StationBBg))]
    [NotifyPropertyChangedFor(nameof(StationCBg))]
    [NotifyPropertyChangedFor(nameof(StationAFg))]
    [NotifyPropertyChangedFor(nameof(StationBFg))]
    [NotifyPropertyChangedFor(nameof(StationCFg))]
    private int _currentStation;

    // 工位高亮：直接绑定 Brush（与 StatusBrush 同模式，避免 MD 样式上下文里 DataTrigger 改背景不生效）

    /// <summary>
    /// 选中工位背景色。
    /// </summary>
    private static readonly Brush StationSelBg = new SolidColorBrush(Color.FromRgb(0x2F, 0x6F, 0xED));

    /// <summary>
    /// 未选中工位背景色。
    /// </summary>
    private static readonly Brush StationUnselBg = new SolidColorBrush(Color.FromRgb(0xE3, 0xE7, 0xEC));

    /// <summary>
    /// 选中工位前景色。
    /// </summary>
    private static readonly Brush StationSelFg = Brushes.White;

    /// <summary>
    /// 未选中工位前景色。
    /// </summary>
    private static readonly Brush StationUnselFg = new SolidColorBrush(Color.FromRgb(0x55, 0x55, 0x55));

    /// <summary>
    /// 冻结工位配色画刷：允许在后台线程构造本 VM（未冻结的 Freezable 有线程亲和，跨线程绑定会失效）。
    /// </summary>
    static TestRunViewModel()
    {
        StationSelBg.Freeze();
        StationUnselBg.Freeze();
        StationUnselFg.Freeze();
    }

    /// <summary>
    /// 工位 A 背景（选中高亮）。
    /// </summary>
    public Brush StationABg => CurrentStation == 0 ? StationSelBg : StationUnselBg;

    /// <summary>
    /// 工位 B 背景（选中高亮）。
    /// </summary>
    public Brush StationBBg => CurrentStation == 1 ? StationSelBg : StationUnselBg;

    /// <summary>
    /// 工位 C 背景（选中高亮）。
    /// </summary>
    public Brush StationCBg => CurrentStation == 2 ? StationSelBg : StationUnselBg;

    /// <summary>
    /// 工位 A 前景（选中高亮）。
    /// </summary>
    public Brush StationAFg => CurrentStation == 0 ? StationSelFg : StationUnselFg;

    /// <summary>
    /// 工位 B 前景（选中高亮）。
    /// </summary>
    public Brush StationBFg => CurrentStation == 1 ? StationSelFg : StationUnselFg;

    /// <summary>
    /// 工位 C 前景（选中高亮）。
    /// </summary>
    public Brush StationCFg => CurrentStation == 2 ? StationSelFg : StationUnselFg;
    ...
    /// 正在测试（全自动循环 或 手动单次）。
    /// </summary>
    public bool IsBusy => IsRunning || IsAutoRunning;
    ...
    /// 当前工位号文字（A/B/C）。
    /// </summary>
    public string CurrentStationText => ((char)('A' + CurrentStation)).ToString();
    ...
    /// <summary>
    /// 批次号：支持手动输入，或「从PLC获取」从 PLC 地址位读取。
    /// </summary>
    [ObservableProperty] private string _batchNumber = "";

    /// <summary>
    /// 从 PLC 地址位获取批次号填入。
    /// PLC 批次号真实对接尚未完成：测试账号（admin）直接读占位/仿真值方便调试；
    /// OA 正式账号则提示核对工单后手动输入，避免占位批次号混入产线数据。
    /// </summary>
    [RelayCommand]
    private async Task FetchBatchFromPlc()
    {
        if (!_session.IsTestAccount)
        {
            AppDialog.Info("从 PLC 获取批次号",
                "PLC 批次号对接尚未完成，暂不支持自动获取。\n请核对生产工单后，在批次号输入框中手动输入。");
            return;
        }

        try
        {
            BatchNumber = await _plc.ReadBatchNumberAsync();
            _notify.Notify($"已从 PLC 获取批次号：{BatchNumber}");
        }
        catch (Exception ex) { _notify.Notify($"从 PLC 获取批次号失败：{ex.Message}"); }
    }
    ...
    private RunOptions BuildOptions()
    {
        ...
        return new RunOptions
        {
            ...
            // 记录当前工位（自动/手动均记录操作员所选工位 A/B/C）
            StationNo = CurrentStationText,
    ...
    /// <summary>
    /// 开始任务前置校验：自动连接共享设备（标准盒；自动上下料时还含 PLC），连接失败才提示去手动配置；
    /// 再校验批次号。通过返回 true。
    /// </summary>
    /// <returns>校验通过返回 true。</returns>
    private async Task<bool> ValidateBeforeStartAsync()
    {
        // 1. 自动连接共享设备（标准盒两种模式都需要；PLC 仅自动上下料模式）
        try { await _conn.ConnectBoxAsync(); } catch { }
        if (AutoMode)
        {
            try { await _conn.ConnectPlcAsync(); } catch { }
        }

        BoxConnected = _conn.IsBoxConnected;
        PlcConnected = _conn.IsPlcConnected;

        if (!_conn.IsBoxConnected || (AutoMode && !_conn.IsPlcConnected))
        {
            AppDialog.Error("无法开始测试",
                $"共享设备（标准盒{(AutoMode ? "、PLC" : "")}）连接失败，当前共享设备连接配置信息可能无效。\n" +
                "请打开【连接配置】手动配置正确后再连接。");
            return false;
        }
        ...
        {
            AppDialog.Info("无法开始测试", "请先填写批次号（可手动输入或从 PLC 获取）。");
            return false;
        }
        return true;
    }
    ...
    /// <summary>
    /// 开始/结束 合一：未运行时按 全自动开关 启动（开=全自动 PLC 循环，关=手动单次）；
    /// 运行中则停止。每次开始自动重置计数与号位数据。
    ...
    private async Task StartStop()
    {
        ...
        if (IsBusy)
        {
            _plc.IsWatching = false;
            _runCts?.Cancel();
            return;
        }

        // 开始前校验：自动连接共享设备（自动上下料含 PLC）+ 批次号
        if (!await ValidateBeforeStartAsync())
        {
            return;
        }

        // 自动上下料模式：开始前确认 模式 + 工位号
        if (AutoMode)
        {
            var confirm = AppDialog.Confirm("开始全自动测试",
                $"当前为【自动上下料】模式，工位：{CurrentStationText}，批次号：{BatchNumber}。\n确认无误后开始全自动测试？");
            if (!confirm)
            {
                return;
            }
        ...
        if (AutoMode)
        {
            // 全自动：接入 PLC 自动上下料循环
            IsAutoRunning = true;
            _notify.Notify($"{_manifest.BoardName} 全自动测试开始");
            _ = Task.Run(async () =>
            {
                try
                {
                    await _orch.RunFullAutoAsync(_manifest, _counters, BuildOptions(), ct,
                        onResult: persist ? (r => _store.SaveAsync(r, ct)) : null,
                        onCycleStart: sns => _dispatcher.Invoke(() => StartNewCycle(sns)),
                        onSchedule: msg => _dispatcher.BeginInvoke(() => _notify.Notify("PLC调度：" + msg)),
                        // 测试中途停止且工位仍有压合板：弹窗确认是否清料（测试不完整将按 NG 下料）
                        confirmCleanupOnStop: _ => Task.FromResult(_dispatcher.Invoke(() =>
                            AppDialog.Confirm("测试已中止",
                                $"工位 {CurrentStationText} 仍有已压合的板卡，测试未完整将按 NG 处理。\n是否清理当前工位物料（松开针床并下料）？"))));
                }
                catch (OperationCanceledException) { }
                finally { _dispatcher.Invoke(() => { IsAutoRunning = false; StopLingeringSteps(); _notify.Notify($"{_manifest.BoardName} 全自动测试结束"); }); }
            });
        }
        else
        {
            ...
            try
            {
                ...
                var result = await Task.Run(() => _runner.RunAsync(_manifest, BuildOptions(), ct));
                if (persist)
                {
    ...
    /// 选择工位号（运行中禁止切换）。
    /// </summary>
    /// <param name="station">工位号索引字符串。</param>
    [RelayCommand]
    private async Task SelectStation(string? station)
    {
        if (IsBusy)
        {
            // 运行中不允许切换工位号
        ...
        if (!int.TryParse(station, out var idx))
        {
            return;
        }

        await _plc.SelectStationAsync(idx);
        CurrentStation = idx;
        OnPropertyChanged(nameof(CurrentStationText));
    }
    ...
    /// <summary>
    /// PLC 标签文本：真机模式显示"PLC（国锐）"，仿真调试显示"PLC_国锐_调试"。
    /// </summary>
    public string PlcDisplayText => _useReal ? "PLC（国锐）" : "PLC_国锐_调试";
